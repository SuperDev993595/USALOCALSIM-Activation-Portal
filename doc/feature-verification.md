...
| 5 | **SIM Only** – ICCID → validate → Plan Menu → Hardware Discount → Stripe (credit card) | ✅ | `/api/validate?iccid=...` returns plans with `priceCents` reduced by `SIM_HARDWARE_COST_CENTS`. Stripe Checkout uses `payment_method_types: ["card"]` only. |
...
| 9 | **Stripe** – Credit card payments | ✅ | Checkout session uses **card** only; UI copy in all locales matches (no crypto / no bank transfer in Checkout). |
...
| 3 | **Redeemed** – Code "killed", log who used it (Email or ICCID) | ⚠️ | On submit, voucher set to `redeemed`; `redeemedBy` is the customer **email** (combo ICCID is stored on `ActivationRequest.iccid`). One-time use enforced. |
...
| 13 | **Logs** – Every action: who, time, device | ⚠️ | `AuditLog` on unlock, submit, webhook, import, complete (metadata: `ip`, `userAgent`, timestamps). Not every endpoint is audited. |
...
- **Fully implemented:** Project-description items align with the app; instructions items except manual-vs-automation conflicts and strict “every action” logging.
- **Stripe:** Credit card only in Checkout; copy matches **project description.md**.
