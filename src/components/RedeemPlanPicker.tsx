...
export function RedeemPlanPicker({
  ...
  function planPriceLabel(p: RedeemPlanRow, isSelected: boolean) {
    ...
    if (p.matchesVoucherCredit || p.fullyCoveredByWallet) {
      return (
        <span
    ...
    return (
      <span className="text-sm font-semibold tabular-nums text-white">${(p.priceCents / 100).toFixed(2)}</span>
    );
  }
          ...
          {plans.map((p) => {
            ...
            return (
              ...
              >
                ...
                <div className={`min-w-0 flex-1 ${isSelected ? "pr-6" : ""}`}>
                  <p className="text-sm font-semibold leading-snug text-white sm:text-[15px]">{p.name}</p>
                  <p className="mt-0.5 text-xs text-slate-400">
                    {p.dataAllowance} · {p.durationDays} {t("daysSuffix")}
                  </p>
                </div>
                <div className="shrink-0 text-right">{planPriceLabel(p, isSelected)}</div>
              </button>
            );
          })}
        </div>
