...
export function ShopCheckoutReturnClient() {
  ...
  useEffect(() => {
    ...
    async function poll() {
      while (!cancelled && Date.now() - started < maxMs) {
        const res = await fetch(`/api/shop/purchase/status?session_id=${encodeURIComponent(sid)}`);
        ...
        if (data.paid && data.confirmed && data.purchaseId) {
          if (!cancelled) {
            window.location.replace(`/shop/paid?purchaseId=${encodeURIComponent(data.purchaseId)}`);
          }
          return;
        }
  ...
  if (!sessionId) {
    return (
      <div className="ui-card mx-auto max-w-md p-6 text-center">
        ...
        <Link href="/shop/plans" className="btn-primary mt-4 inline-block px-5 py-2 text-sm">
          {t("goPlans")}
        </Link>
      </div>
    );
