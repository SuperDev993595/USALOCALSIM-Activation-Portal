...
export function ShopPlansClient({ plans }: { plans: ShopPlanRow[] }) {
  ...
  async function checkoutMercadoPago() {
    ...
    try {
      const res = await fetch("/api/shop/checkout/mercadopago", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ planId, email }),
      });
  ...
  return (
    <div className="mx-auto w-full max-w-lg">
      <div className="mb-6">
        <Link href="/shop" className="text-sm text-[#00104E] underline">
          {t("backShop")}
        </Link>
      ...
      <div className="ui-card mt-6 p-5">
        <label className="mb-1 block text-sm font-medium text-slate-800" htmlFor="shop-email">
          {t("emailLabel")}
        </label>
        <input
          id="shop-email"
