...
import { sendAdminUserDeleteCodeEmail } from "@/lib/email";
import { createAdminDeleteChallenge } from "@/lib/admin-user-delete-verification";
...
export async function POST(req: Request) {
  ...
  if (body.targetUserId === session.user.id) {
  ...
  const { code, challenge } = createAdminDeleteChallenge(session.user.id, target.id);
  const sent = await sendAdminUserDeleteCodeEmail(actor.email, code, target.email ?? "(no-email)");
  if (!sent.ok) {
